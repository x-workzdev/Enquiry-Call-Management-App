...
public class EnquiryController {
	...
	@RequestMapping(value = "/getCustomEnquiriesForReview.do", method = RequestMethod.POST,produces = "application/json")
	@ResponseBody
	public List<EnquiryCallEntity> getCustomEnquiriesForFillter(@RequestParam("fromDate") String fromDate,@RequestParam("toDate") String toDate) {
		logger.debug("invoked getCustomEnquiriesForFillter() in controller");
		List<EnquiryCallEntity> enquiryList = null;
		try {
			if (Objects.nonNull(fromDate) && Objects.nonNull(toDate)) {
				 enquiryList = enquiryService.getCustomEnquiries(fromDate, toDate);
				if (Objects.nonNull(enquiryList)) {
					logger.debug("Custom Enquiries Fetched Successfully");
					return enquiryList;
				} else {
